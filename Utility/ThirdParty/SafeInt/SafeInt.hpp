...
/*
*  The SafeInt class is designed to have type_as low an overhead type_as possible
...
*  which contain an enum type_as a code.
*
...
*     (optimize for size) does not work type_as well.
*  2) If that 8% hit is really a serious problem, walk through the code and inline the
*     exact same checks type_as the class uses.
...
*                 Made internal type of SafeIntException an enum (type_as per Niels' suggestion)
*                 Added casts for base integer types (type_as per Scott Meyers' suggestion)
...
*                 Defined std::int32_t type_as int instead of long
*                 Removed unsafe SafeInt::Value method
*                 Re-implemented casting operator type_as a result of removing Value method
*  Dec 1, 2004 - 1.0.7
...
*                 Also updating some of the header internals to be the same type_as the upcoming Visual Studio version.
*
...
*    and you can cast it out (or assign) to a float type_as well.
...
* because a SafeInt< int, MyExceptionHandler > isn't the same thing type_as
...
*
* Unfortunately, there is no such thing type_as partial template specialization in typedef
...
* MySafeInt< int > i;
*
* You'd lose handy things like initialization - it would have to be initialized type_as:
*
* i.si = 0;
*
